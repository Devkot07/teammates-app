...
class UpdateUserProfileUseCase @Inject constructor(
    ...
    suspend operator fun invoke(nickname: String, description: String): Result<User> {
        return runCatching {
            ...
            userDataRepository.updateUserProfile(response.nickname, response.description, response.imagePath)
    ...
    fun validateUserProfileForm(
        ...
        return when {
            ...
            description != null && description.length > 2000 -> ValidationResult.Error(ValidationError.DESCRIPTION_TOO_LONG)
    ...
    fun updateUserAvatar(image: MultipartBody.Part) {
        Log.d("Upd", image.toString())
    }
}